...
(select  salary from employees where emp_name = 'Mary');
...
delimiter &&
create procedure top_playernew3()
begin
select name, football_id
from football where football_id <6 ;
End &&
delimiter ;
...
begin
select name, age, salary from employees_1
...
delimiter //
create procedure sp_CountEmployees1(OUT Total_Emps int)
begin
select count(name) into Total_Emps from employees_1
where sex= 'F';
end //
delimiter ;

call sp_CountEmployees1(@F_emp);
